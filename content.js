...
function scrapeStakeData() {
    ...
    if (oddsButtons.length === 0) throw new Error("Data market tdk ditemukan");
    var data = [];
    oddsButtons.forEach(function(btn) {
        var name = btn.querySelector('[data-testid="outcome-button-name"]');
        var odd = btn.querySelector('[data-testid="fixture-odds"]');
...
async function getLiveH2H() {
    ...
    if (idx !== -1) return "H2H: " + text.substring(idx, idx + 1000).replace(/\n/g, " | ");
    return "H2H: tdk ada";
}
