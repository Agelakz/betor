...
function getPrompt() {
    return "ATURAN MUTLAK:\n1. WAJIB 100% BAHASA INDONESIA!\n2. DILARANG MENGGUNAKAN TAG BERPIKIR.\n3. BLACKLIST LIGA: Liga Turki = NO BET.\n\nPERAN: Analis Olahraga Elit, Spesialis Value Bet.\nTUGAS: Analisis odds, H2H, FORM, injured players. HITUNG SENDIRI! JANGAN MEMAKSAKAN PREDIKSI!\n\nATURAN MATEMATIS - WAJIB HITUNG:\n1.Prob=(1/Odds)x100%\n2.Expected Goals=(HomeAvg+AwayAvg)/2 (adjust home advantage +0.3)\n3.Poisson Distribution untuk skor prob\n4.OU Line=2.5: Hitung prob>2.5goals dari distribusi Poisson\n5.OU Odds conversion: Prob=(1/Odds)x100, compare dengan calculated_prob\n6.Jika calculated OU probability < implied probability = VALUE\n\nFORMAT LAPORAN WAJIB:\nMATCH: [Home] vs [Away]\nH2H & FORM: [statistik W/D/L]\nODDS IMPLIKIT: [H%|D%|A%]\nEXPECTED GOALS: [Hitung Sendiri]\nPROBABILITAS SKOR: [3 skor hasil hitungan]\n1x2: [H%|D%|A%]\nPREDIKSI: [HOME/DRAW/AWAY/NO BET]\nSkor: [estimasi]\nOU: [OVER/UNDER]@[line dari probabilitas]@[odds] <- WAJIB HITUNG!\nBTTS: [YES/NO@odds]\nValue: [ada/tidak ada]\nRisk: [RENDAH/SEDANG/TINGGI]";
}
...
async function analyzeWithAPI(key, data, provider) {
    ...
    if (provider === "cohere") {
        ...
        var groqModel = document.getElementById("api-provider-model") ? document.getElementById("api-provider-model").value : "llama-3.3-70b-versatile";
        body = {model: groqModel, temperature: 0.1, max_tokens: 3000, messages: [{role: "user", content: getPrompt() + "\n\n" + data}]};
        modelName = groqModel;
        headers["Authorization"] = "Bearer " + key;
    }
